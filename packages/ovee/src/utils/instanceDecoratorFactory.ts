...
export type ExtractDecorator<T, F extends DecoratorFactoryCallback<T>> =
    (...args: ExtractRest<F>) => (target: any, prop: string) => void
...
export type DecoratorFactoryCallback<T> =
    (ctx: DecoratorContext<T>, prop: string, ...rest: any[]) => any;
...
function instanceDecoratorFactory<T, Cb extends DecoratorFactoryCallback<T>>(callback: Cb): ExtractDecorator<T, Cb> {
    return (...args) => (target, prop) => {
        ...
        ctor[protectedFields.INSTANCE_DECORATORS]!.push(
            (instance) => callback({ instance, addDestructor }, prop, ...args)
        );
    };
}
